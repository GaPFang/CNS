...
def main():
    for _ in range(200):
        ...
        mt = MT19937()
        output_for_cloning = []
        line = r.recvuntil(b'number: ').strip().decode()
        if 'your number:' in line:
            r.sendline(b'0')
            continue
        while True:
            line = r.recvuntil(b'number: ').strip().decode()
            if 'your number:' in line:
                if count < 624:
                    ...
                    mt.clone_state_from_output(output_for_cloning)
                    future_output = []
                    for _ in range(userIdx - 630):
                        future_output.append(mt.get_next_random())
                    # print(future_output)
                    for _ in range(800 - userIdx):
                        future_output.append(mt.get_next_random())
                    # print(future_output)
                    mt.rewind(799)
                    previous_output = []
                    for _ in range(5):
                        previous_output.append(mt.get_next_random())
                    ballot = sum(previous_output) + sum(future_output) + sum(output_for_cloning)
                    # print(len(previous_output) + len(future_output) + len(output_for_cloning))
                    myChoice = (userIdx - 1 - ballot + 800) % 800
                    # print(myChoice)
                ...
                cur = int(line.split('player ')[1].split('\'')[0]) - 1
                if count < 624:
                    number = int(line.split('\nplayer')[0])
                    output_for_cloning.append(number)
                    count += 1
